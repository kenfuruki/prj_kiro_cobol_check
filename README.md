...
COBOLの開発プロジェクトにこのツールを組み込む場合、Agent Skills仕様に準拠した以下の構成で配置してください。

```
your-cobol-project/                       ← COBOLプロジェクトのルート
├── .kiro/
│   ├── skills/
│   │   └── cobol-static-check/           ← ① スキルフォルダ（name と一致）
│   │       ├── SKILL.md                  ←    スキル定義（必須）
│   │       └── scripts/
│   │           └── cobol_static_checker/ ←    チェッカー本体をここにコピー
│   │               ├── __init__.py
│   │               ├── normalizer.py
│   │               ├── variable_parser.py
...
1. 本リポジトリの `.kiro/skills/cobol-static-check/` フォルダをプロジェクトの `.kiro/skills/` にコピーする
2. スキルフォルダ内に `scripts/cobol_static_checker/` を作成し、本リポジトリの `cobol_static_checker/` 配下のファイルをコピーする
3. `SKILL.md` 内のコマンドパスをプロジェクトに合わせて修正する
4. EXE版が必要な場合は `dist/cobol_checker.exe` を配置する（Python不要の端末向け）
...
```bash
PYTHONPATH=.kiro/skills/cobol-static-check/scripts python -m cobol_static_checker.cli cobol_src -o check_result.csv
```
